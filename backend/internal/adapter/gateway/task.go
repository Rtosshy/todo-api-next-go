...
func NewTaskRepository(db *gorm.DB) repo.TaskRepo {
	return &taskRepoImpl{db: db}
}

func (tr *taskRepoImpl) GetOrCreateStatus(task *domain.Task) error {
...
func (tr *taskRepoImpl) Create(task *domain.Task) (*domain.Task, error) {
...
func (tr *taskRepoImpl) Get(taskID domain.TaskID, userID domain.UserID) (*domain.Task, error) {
...
func (tr *taskRepoImpl) GetAll(userID domain.UserID) (*[]domain.Task, error) {
	tasks := []domain.Task{}
	if err := tr.db.Preload("Status").Preload("User").
...
func (tr *taskRepoImpl) Save(task *domain.Task) (*domain.Task, error) {
...
func (tr *taskRepoImpl) Delete(taskID domain.TaskID, userID domain.UserID) error {
	var task = domain.Task{}
