...
- Redis: 高速なインメモリデータストアであり、キャッシュやセッション管理には適している。また、`MULTI/EXEC`やLua scriptによる原子的な実行も可能。ただし、User / Task / Status 間の関係性を外部キー制約やリレーショナルなスキーマで担保する主DB用途には向かないため不採用。
